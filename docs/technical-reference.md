...
- **One `lua_State` for the entire session.** Empirical: the proxy's `lua_newstate` / `luaL_newstate` counter shows exactly 1 state created across cold launch → main menu → game setup → in-game world map. Civ V does NOT create a separate state for in-game; front-end and in-game share one state, distinguished only by the set of Contexts currently alive. Prior hypothesis ("one state per phase") was wrong — do not rely on state identity to tell phases apart. `civvaccess_shared` consequently persists across phases for free.
- **Phase detection: use `Events.LoadScreenClose`, not state identity.** Boot.lua is an `InGameUIAddin` but fires in the front-end too — `Modding.ActivateEnabledMods()` during proxy auto-enable loads it once, and the game-setup flow loads it again, both in the front-end phase. The reliable "we are in a real game now" signal is `Events.LoadScreenClose`. Boot.lua registers an `onInGameBoot` listener on it instead of running its body inline.
- **Mod TXT_KEY tables don't get ingested via `Modding.ActivateEnabledMods()`.** Empirical: our mod's `Text/CivVAccess_Text_en_US.xml` is listed under `<GameData><Text>` in the modinfo and the file is copied, but `Locale.ConvertTextKey("TXT_KEY_CIVVACCESS_*")` returns the raw key at runtime — both on the front-end and in-game. Nothing appears in `modding.log` / `Localization.log` either. The Lua-driven enable/activate path skips the normal mod-setup content-ingestion step. Until a followup addresses this, mod-authored in-game announce strings are inline literals, not `Text.key` lookups. Game's own `TXT_KEY_*` entries still work — they're in the base text tables, not in our mod's XML.
- **Cross-Context shared state: `civvaccess_shared`.** The proxy registers an empty table as a global named `civvaccess_shared` at `luaL_openlibs` time and injects it into every sandbox env during `lua_setfenv`, mirroring the `tolk` injection. HandlerStack (and any future module that needs cross-Context state within a phase) reads/writes this table instead of module locals. Module-local state fragments because each Context `setfenv`-sandbox gets its own copy of every `include`d module; the shared table is a single table-identity referenced from every sandbox.
...
- **Front-end coverage options:**
  ...
  2. Proxy-time script injection via the `setfenv` hook for front-end Lua contexts. No Civ V mod system involvement. **This is what we do.** The proxy detects front-end envs (they lack `Modding`) and loads the ten `CivVAccess_*.lua` modules from a sibling `<gameDir>\CivVAccess\` directory via `luaL_loadbuffer` + `setfenv`, then pushes `BaselineHandler` and speaks `TXT_KEY_CIVVACCESS_BOOT_FRONTEND`. A sentinel `civvaccess_bootstrapped` on the env prevents re-runs. In-game envs get the sentinel set without module loading; Boot.lua's `include` chain handles them.
