...
| `S` | Baseline in-game | Speak the top unit on the cursor tile (military unit first, civilian fallback) | Replaces `Set up for ranged attack`, `Air strike`, `Scrub fallout` -- all unit/worker missions that fire only when a relevant unit is selected. Reading the cursor tile's occupant is the single most-pressed query during navigation, so it takes the modifier-free slot; the previous plain-S binding (orient from capital) moves to Shift+S. |
| `Shift+S` | Baseline in-game | Speak distance and direction from the player's capital | No engine binding on Shift+S. Inherits the orient-from-capital announcement that plain S used to own; kept one modifier away so the cursor-key vocabulary stays intact. |
