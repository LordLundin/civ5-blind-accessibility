...
| `Ctrl+Shift+F12` | Every Context that routes through InputRouter | Hotseat hard-mute toggle. Enter mute: cuts in-flight speech and silences all subsequent speech and key interception so the keyboard reaches the engine for a sighted hotseat partner. Exit mute: restores both and speaks "resumed" as the only audible confirmation. | No engine binding on any `F12` chord. Wired as the first pre-walk hook in `InputRouter.dispatch` so the toggle works in both states; gates dispatch on `civvaccess_shared.muted` so HandlerStack push/pop continue (observers, ChatBuffer, NotificationAnnounce, RevealAnnounce keep their state current) and unmuting resumes from a live stack. SpeechPipeline reads the same shared flag so every Context's speech is silenced uniformly. |
