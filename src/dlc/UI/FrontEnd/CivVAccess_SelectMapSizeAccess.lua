...
local function buildItems()
    local items = {}
    local randomLabel = PreGame.IsMultiplayerGame() and "TXT_KEY_ANY_MAP_SIZE" or "TXT_KEY_RANDOM_MAP_SIZE"
    local randomHelp = PreGame.IsMultiplayerGame() and "TXT_KEY_ANY_MAP_SIZE_HELP" or "TXT_KEY_RANDOM_MAP_SIZE_HELP"
    items[#items + 1] = BaseMenuItems.Choice({
        labelText = Text.key(randomLabel),
        tooltipText = Text.key(randomHelp),
        visibilityControl = g_RandomSizeControl.Root,
        selectedFn = function()
            return PreGame.IsRandomWorldSize()
        end,
        activate = function()
            SizeSelected(-1)
        end,
    ...
    for info in GameInfo.Worlds() do
        ...
        items[#items + 1] = BaseMenuItems.Choice({
            ...
            selectedFn = function()
                return not PreGame.IsRandomWorldSize() and PreGame.GetWorldSize() == id
            end,
            activate = function()
                SizeSelected(id)
            end,
