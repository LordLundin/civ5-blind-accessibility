...
- `deploy.ps1` — copies the committed artifacts into the game install: `dist/proxy/lua51_Win32.dll`, the Tolk runtime DLLs from `third_party/tolk/dist/x86/`, the `src/dlc/` payload into `Assets/DLC/DLC_CivVAccess/`, `sounds/*.wav` into the DLC's `Sounds/`, and (with `-DeployEngine`) `dist/engine/CvGameCore_Expansion2.dll`. Engine DLL deploy is opt-in; on first install it backs up the vanilla DLL to `build/CvGameCore_Expansion2.vanilla.dll.bak`. `-Uninstall` reverses everything (restoring the vanilla engine DLL if a backup exists).

Daily iteration when only Lua / DLC payload changed: `./deploy.ps1`. After touching `src/proxy/`: `./build-proxy.ps1; ./deploy.ps1`. After touching `src/engine/`: `./build-engine.ps1; ./deploy.ps1 -DeployEngine`. Always use the scripts; never invoke `cl.exe` / `lua` / copy commands directly.
...
- `src/dlc/` — fake-DLC payload. One BNW-only manifest (`CivVAccess_2.Civ5Pkg`, `<UISkin set="Expansion2">`), per-Context UI under per-Context directories (e.g. `UI/InGame/`, `UI/FrontEnd/`, `UI/TechTree/`) routed by the manifest's `<Skin>` (front-end) and `<GameplaySkin>` (in-game) directives, and cross-Context infra under `UI/Shared/` (listed under both directives so either Context can `include` from it). Mod-authored locale strings live in per-Context files with distinct include stems because `include()` caches by bare stem on the shared lua_State. BNW is a hard requirement: if the active UISkin isn't Expansion2 the engine skips the manifest and the mod never loads — no fallback to base / G&K.
...
- **Engine bindings added by the fork only resolve on a fork deploy.** Any Lua method `src/engine/` adds (currently in `CvGameCoreDLL_Expansion2/Lua/CvLuaUnit.cpp` and `CvGameCoreDLL_Expansion2/Lua/CvLuaGame.cpp`, marked with `CIVVACCESS:` comments) is missing or stubbed in the vanilla DLL — `Unit:GeneratePath` is shipped as `luaL_error("NYI")`, the others aren't registered at all. Lua callers that depend on these must either be guarded by a feature check or the contributor must run `deploy.ps1 -DeployEngine` (after `build-engine.ps1` if the overlay was just changed). When adding a new binding, check the existing `CIVVACCESS:` markers for the registration pattern; the overlay is full-file replacement (no patch mechanism), so the file you modify must be a verbatim copy of the SDK source plus your additions.
